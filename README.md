...
## 버전
> Java 11, Spring Boot 2.5.4, gradle 6.9.1 기반 > aws 배포환경 현재 java11까지만 지원 >  amazonaws:aws-java-sdk - 1.12.131 (21년 12월 버전) >   aws-encryption-sdk-java - 2.0.0 (20년 8월)
...
### crypto
- `CryptoService` : kms 사용 암호화/복호화
- Sha256 hash

### csv
csv 파일 생성 (`CsvGeneratorTest` 사용 확인)
...
### excel
- 데이터 -> 엑셀파일 생성    
  시트별 생성/ 엑셀 헤더(굵기,폰트사이즈,배경색,글자색) 스타일 지정 - 엑셀파일 -> 데이터 추출    
  시트별 추출- `ExcelGeneratorTest` 사용 확인 #### security
- 인증 로직
...
> 해당 api모듈만 배포 현재 user-api 배포시 `buildspec.yml` 사용
